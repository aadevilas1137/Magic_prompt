'use client';

import { useTranslations } from 'next-intl';
import { useState } from 'react';
import { toast } from 'sonner';

import { Button } from '@/components/ui/button';
import { env } from '@/lib/env';
import { createClient } from '@/lib/supabase/client';
import { cn } from '@/lib/utils';

interface GoogleOAuthButtonProps {
  readonly redirectTo?: string | undefined;
  readonly className?: string | undefined;
  readonly children?: React.ReactNode;
}

export function GoogleOAuthButton({ redirectTo, className, children }: GoogleOAuthButtonProps) {
  const t = useTranslations('auth');
  const [pending, setPending] = useState(false);

  async function handleClick() {
    setPending(true);
    try {
      const supabase = createClient();
      const callbackUrl = new URL('/auth/callback', env.NEXT_PUBLIC_APP_URL);
      if (redirectTo) callbackUrl.searchParams.set('redirect', redirectTo);
      const { data, error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: { redirectTo: callbackUrl.toString() },
      });
      if (error) throw error;
      if (data.url) {
        window.location.assign(data.url);
        return; // navigation happens; don't reset state
      }
      throw new Error('No OAuth URL returned from provider');
    } catch (err) {
      console.error('[oauth-google]', err);
      toast.error('Could not start Google sign-in. Please try again.');
      setPending(false);
    }
  }

  return (
    <Button
      type="button"
      variant="outline"
      onClick={handleClick}
      disabled={pending}
      className={cn('w-full', className)}
    >
      {pending ? (
        <span className="inline-flex items-center gap-2">
          <span
            className="border-muted-foreground h-4 w-4 animate-spin rounded-full border-2 border-t-transparent"
            aria-hidden
          />
          {t('googlePending')}
        </span>
      ) : (
        <span className="inline-flex items-center gap-2">
          <GoogleLogo />
          {children ?? t('google')}
        </span>
      )}
    </Button>
  );
}

function GoogleLogo() {
  return (
    <svg viewBox="0 0 24 24" className="h-4 w-4" aria-hidden>
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.76h3.56c2.08-1.92 3.28-4.74 3.28-8.09Z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.56-2.76c-.99.66-2.25 1.06-3.72 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84A11 11 0 0 0 12 23Z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.11A6.6 6.6 0 0 1 5.5 12c0-.73.13-1.44.34-2.11V7.05H2.18a11 11 0 0 0 0 9.9l3.66-2.84Z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.13 14.96 1 12 1 7.7 1 3.99 3.47 2.18 7.05l3.66 2.84C6.7 7.31 9.13 5.38 12 5.38Z"
      />
    </svg>
  );
}
